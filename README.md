...
| `POST` | `/device/payment-request` | Create a Bayar QR Online payment request (FR20) | Device |
| `POST` | `/device/payment-request/{requestId}/pay` | Pay a Bayar QR Online payment request; device-generated Idempotency-Key required (FR21) | Device |
...
- [x] **PR21 — FR28-adjacent — Phase 3 Feature A: Notification Reconciliation** — `notification_log` (Flyway `V10__notification_log.sql`) tracks `wallet/{deviceId}/payment-received` delivery for OFFLINE_TRANSFER settlements only; worker inserts PENDING + attempts immediate publish right after each OFFLINE_TRANSFER commits; any authenticated hit on `POST /device/sync` or `POST /device/pouch/load` re-attempts a device's PENDING rows (no new "I'm online now" endpoint) via `MqttAdminClient` in the api profile; hourly ShedLock `NotificationExpiryJob` marks stale PENDING rows EXPIRED after `notification.reconciliation.expiry-days` (default 3); purely delivery metadata — `GET /device/balance` never references this table, verified by a dedicated test that pauses a real Mosquitto broker for the entire settlement+balance-check flow
