...
- **Device holder (payer/payee).** Owns a Dompet device, tops up online, transacts offline. Interacts via the device's screen + buttons and a PIN.
...
See §2a. In short: no real money, no real QRIS/bank rail, no compliance, no scaling, no consumer app polish, no multi-hop offline re-spend (unless added deliberately). Build §4 and nothing beyond it.
...
4. The full stack runs within the 8 GB box without swapping under demo load.
5. A backup can be **restored** successfully (tested at least once before the demo).
...
4. API service built (Claude Code). ◀ current
