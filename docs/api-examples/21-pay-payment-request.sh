...
#               Payer needs a registered device token (05-register-device.sh) and
#               sufficient online balance (03-topup.sh).

BASE_URL="${BASE_URL:-http://localhost:8080}"
PAYER_DEVICE_TOKEN="replace-with-payer-device-token"
...
curl -s -X POST "${BASE_URL}/device/payment-request/${REQUEST_ID}/pay" \
  -H "Authorization: Bearer ${PAYER_DEVICE_TOKEN}" \
