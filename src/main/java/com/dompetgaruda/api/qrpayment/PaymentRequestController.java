...
/**
 * FR20/FR21 — Bayar QR Online. Both create (receiver) and pay (payer) identify the caller via
 * the same {@code Device-Id} header (CLAUDE.md §1b, PRD R20) — which role applies is determined
 * by which endpoint is called, not by the header name. No device Bearer token.
 *
...
public class PaymentRequestController {
    ...
    public CreatePaymentRequestResponse create(
            @Parameter(description = "Receiving device's id (CLAUDE.md §1a, §1b). Required — identifies the receiving device and its owning user. Role (receiver) is determined by this endpoint, not the header name.", required = true)
            @RequestHeader(name = "Device-Id", required = false) String receiverDeviceId,
    ...
    public ResponseEntity<PayPaymentRequestResponse> pay(
            @Parameter(description = "Paying device's id (CLAUDE.md §1a, §1b). Required — identifies the paying device and its owning user. Role (payer) is determined by this endpoint, not the header name.", required = true)
            @RequestHeader(name = "Device-Id", required = false) String payerDeviceId,
